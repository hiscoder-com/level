...
export default function Recorder() {
  ...
  const [buttonRecord, setButtonRecord] = useState(
    <RecorderButton className="stroke-th-primary-icons stroke-2" />
  )
  const [buttonPlay, setButtonPlay] = useState(
    <PlayButton className="stroke-th-primary-icons stroke-2" />
  )
  ...
  const startStop = () => {
    if (mediaRec?.state === 'inactive') {
      setVoice([])
      mediaRec.start()
      setButtonRecord(<StopButton className="stroke-th-primary-icons stroke-2" />)
      setInactive(true)
    } else if (mediaRec?.state === 'recording') {
      mediaRec.stop()
      setButtonRecord(<RecorderButton className="stroke-th-primary-icons stroke-2" />)
      setInactive(false)
    } else {
      setShowModal(true)
    }
  ...
  useEffect(() => {
    if (voice.length > 0) {
      setButtonPlay(<PlayButton className="stroke-th-primary-icons stroke-2" />)
      const blobUrl = window.URL.createObjectURL(
        new Blob(voice, { type: 'audio/webm;codecs=opus' })
      )
      audioRef.current.src = blobUrl
    } else if (audioRef.current) {
      setButtonPlay(<PlayButton className="stroke-th-disabled-text stroke-2" />)
  ...
  const playPause = () => {
    if (audioRef.current.paused) {
      ...
      setButtonPlay(<PauseButton className="stroke-th-primary-icons stroke-2" />)
    } else {
  ...
  return (
    <div className="flex flex-row items-center gap-7">
      ...
      >
        <TrashButton
          className={`stroke-2 ${
            voice.length > 0 ? 'stroke-th-primary-icons' : 'stroke-th-disabled-text'
          }`}
        />
      </button>
