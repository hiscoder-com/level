...
function SwitchLocalization() {
  ...
  return (
    <div className="text-xs lg:text-sm font-bold relative">
      <Menu>
        <Menu.Button
          className="px-4 py-2 text-sm bg-th-primary-background rounded-[9rem]"
          onClick={(e) => e.stopPropagation()}
        >
          {t(locale.toUpperCase())}
        </Menu.Button>
        <Menu.Items className="absolute flex top-0 right-0 text-sm bg-th-primary-background rounded-2xl">
          <div className="flex flex-col">
            {supportedLngs.map((loc) => (
              ...
              >
                <div className={`${locale === loc ? 'text-th-disabled-text' : ''}`}>
                  {t(loc.toUpperCase())}
                </div>
              </Menu.Item>
            ))}
